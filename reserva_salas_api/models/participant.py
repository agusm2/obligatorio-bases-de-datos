...
class Participant:
        ...
        try:
            ...
            if password is not None:
                if not obj.email:
                    ...
                    raise ValueError(
                        "email es obligatorio para crear el Login cuando se pasa password"
                    )
                cur.execute(
    ...
    def update(self, email=None):
        ...
        try:
            cur.execute(
                "UPDATE Participante SET email=%s WHERE ci=%s", (new_email, self.ci)
            )
            conn.commit()
    ...
    def delete(self):
        ...
        cur = conn.cursor()
        try:
            cur.execute("DELETE FROM Participante WHERE ci=%s", (self.ci,))
            conn.commit()
        finally:
            cur.close()
            conn.close()
    ...
    def add_sancion(cls, ci, fecha_fin, fecha_inicio=None):
        ...
        return {
            ...
            "fecha_inicio": (
                ...
                if fecha_inicio_dt is not None
                else None
            ),
